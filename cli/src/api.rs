...
/// The RECORDED outcome of one administrative resume cancellation: the body of
/// `POST /approvals/{id}/resume/cancel` (`ApprovalResumeCancelOut`, #2753).
///
/// A distinct type from [`ApprovalRecoveryOutcome`] because the two are
/// distinct administrative acts: this one timestamps `cancelled_at` and leaves
/// `status` untouched, and a shared struct would let either be rendered as the
/// other.
#[derive(Debug, Clone, Deserialize)]
pub struct ApprovalResumeCancellation {
    pub approval_id: String,
    pub status: String,
    pub recovery_key: Option<String>,
    pub reason: Option<String>,
...
impl ApiClient {
    ...
    /// Tombstone one approval's resume turn: `POST
    /// /approvals/{id}/resume/cancel` (#2753).
    ///
    /// Carries NO disposition: cancelling a resume is not a resolution, and a
    /// body field that looked like one would make the durable outcome
    /// ambiguous to a reader of the audit trail.
    pub async fn cancel_approval_resume(
        &self,
        approval_id: &str,
        reason: &str,
        recovery_key: &str,
        principal_token: &str,
    ) -> Result<ApprovalResumeCancellation> {
        let resp = self
            .send_request(
                self.http
                    .post(format!(
                        "{}/approvals/{approval_id}/resume/cancel",
                        self.base_url
                    ))
                    .header("X-API-Key", &self.api_key)
                    .header("X-Curie-Approval-Principal", principal_token)
                ...
                "POST /approvals/{id}/resume/cancel",
            )
            .await?;
        Self::expect_ok(resp, "cancelling approval resume")
            .await?
            .json()
            .await
            .context("decoding the recorded resume cancellation")
    }
