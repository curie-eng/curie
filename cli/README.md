...
| `curie cluster status` | Report release health, exact target-image convergence, pod readiness, and access URLs (read-only). Returns exit 1 when the target has not converged, including an old ready replica beside a failed replacement. `--json` retains the status payload and lists redacted rollout reasons in `pods.unhealthy`. Tagged images reported under a sibling alias require read access to the serving Pod's Node: the requested tag and the reported alias must each uniquely bind to the same manifest digest as the Ready container's image ID. A shorter kubelet alias of that digest is accepted, including when kubelet lists the tag, alias, and digest as separate Node.status.images entries of one repository. A missing name, two digests for the same name, or a digest that does not match the Ready image ID fails closed. A digest-pinned chart request is accepted from the Ready image ID even when Node.status.images omits the name (the k3s 50-image cap). This verifies loaded-image binding, not mutable-tag freshness. |
