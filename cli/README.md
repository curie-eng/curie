...
| `curie cluster upgrade --to <version>` | Resumable upgrade lifecycle: plan, validate, drain, checkpoint, migrate, apply, exact convergence, canary, known-good commit. The operator does not pass Helm merge flags. Success requires exact convergence and an API/storage canary that retains the agent identity set; this canary does not verify bot turns, approvals, or active threads. A failure records the last known-good version and reports recovery instructions, without guaranteeing that version remains serving. `--forward-only` explicitly permits contract or irreversible migrations. `--dry-run` prints an offline redacted plan and states that source compatibility has not been inspected. |
...
On Linux, stopping the transactional upgrade CLI also stops its direct Helm mutation process. Use an ordinary Helm executable for this protection; wrappers, privileged executables, child processes launched by Helm plugins, and Kubernetes hook Jobs are outside it. Transactional upgrade ownership supports Linux and macOS with a private local state directory (`$XDG_STATE_HOME/curie/upgrades`, otherwise `$HOME/.local/state/curie/upgrades`). The command captures one private flattened kubeconfig, reads the namespace UID, and holds one non-expiring ownership file for that release through the direct Helm process lifetime. A second cooperating CLI using the same state directory exits 3 before checkpoint access; retry after the owner finishes and never delete a held ownership file. Every Helm/Kubernetes operation uses the captured context even if the ambient context changes. Namespace read permission is required. Nonempty `HELM_KUBE*` endpoint, context, authentication, impersonation, and TLS overrides are refused before discovery; put those settings in the selected kubeconfig. On macOS, an interrupted CLI does not automatically terminate Helm; ownership remains held while that direct Helm process runs. This does not exclude another host, another state directory, raw Helm, or remote hook work. An interrupted upgrade still requires inspection of the release and hooks before recovery; reacquiring local ownership does not authorize rollback or prove that an in-flight cluster request was cancelled. Private captured credentials are removed on ordinary completion and handled termination; SIGKILL can leave a protected temporary file requiring cleanup.

Transactional upgrades with all three managed drain/migration/release hooks retain those Jobs under a fresh operation UUID. The checkpoint and locked local file bind the source revision/UID, target content and original pending revision. Automatic target-forward recovery is limited to a returned Helm invocation that left a pending revision and whose original invocation observed all three retained Jobs and Pods complete, bound the pending revision UID and content, and whose retry independently observes the target schema already reached. It uses ordinary Helm rollback to that exact target revision with fresh drain, idempotent migration and drain release. Only unrelated test-only hooks are ignored. Additional upgrade or rollback hooks refuse, so default deployments with controller/preflight/backing-service upgrade hooks require a separately reviewed retention/terminal prerequisite. Missing, replaced, active or unknown phase evidence refuses; process death before the original observation and any interrupted rollback remain unsupported automatic recovery cases. The command requires read access to Helm Secret metadata and namespace Jobs/Pods. This is neither distributed exclusion nor a nine-phase interruption guarantee; it does not guarantee the previous version keeps serving.
