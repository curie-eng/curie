//! Integration: the `surfaces` verb at BOTH tiers (ADR-0118, #1525).
//!
...
/// One agent's wire JSON with its plural `channels` array (ADR-0118).
...
fn local_surfaces_add_forwards_a_write_only_reply_route() {
    ...
    let run = run(&[
        ...
        AGENT_NAME,
        "--add",
        "discord=111111111111111111", // gitleaks:allow -- zero-entropy placeholder snowflake, not a real bot/client id
        "--endpoint",
...
fn local_surfaces_json_carries_the_agent_and_its_bindings() {
    ...
    // guessing it, which is the whole shape ADR-0118 introduced.
    assert_eq!(
        value["surfaces"],
        serde_json::json!([
