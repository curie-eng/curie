...
//!
//! Three verbs map one-to-one onto the Stream B routes:
//!
//! - `--report-identity`   -> `GET  /approvals/identity-report`
//! - `--recover <ID>`      -> `POST /approvals/{id}/recover`
//! - `--cancel-resume <ID>`-> `POST /approvals/{id}/resume/cancel`
//!
//! Written failing-first, before the implementation exists. These drive the
...
//! as much as the happy path: a disabled grant, a missing principal and a
//! missing reason are three DISTINCT messages, and none of them may read as a
//! membership or permission-denied failure.
...
/// The body `POST /approvals/{id}/resume/cancel` returns: `ApprovalResumeCancelOut`.
/// Timestamped `cancelled_at`, and `status` is the record's UNCHANGED status,
/// because a tombstone is not a resolution.
fn resume_cancellation_json() -> String {
    format!(
        r#"{{"approval_id":"{APPROVAL_ID}","status":"approved","recovery_key":"{RECOVERY_KEY}","reason":"{REASON}","actor":"U0OPERATOR","cancelled_at":"2026-09-18T10:05:00Z"}}"#
    )
}
...
/// Every tier that serves the durable approvals handler must expose all three
...
#[test]
fn local_and_cluster_expose_the_three_recovery_verbs() {
...
/// The two recovery verbs are separate administrative acts against the same
/// row; combining them in one invocation makes the durable outcome ambiguous.
#[test]
fn recover_and_cancel_resume_refuse_each_other() {
    let server = no_request_server();
    let output = run(
        "local",
        &[
    ...
    assert_eq!(
        output.status.code(),
        Some(2),
        "--recover and --cancel-resume must not combine; output:\n{combined}"
    );
    assert!(
        combined.contains("--recover") && combined.contains("--cancel-resume"),
        "the refusal must name both verbs; output:\n{combined}"
    );
}
...
fn recovery_verbs_refuse_a_missing_or_empty_reason_before_any_request() {
    ...
    let cases: &[(&str, Vec<&str>)] = &[
        ...
        (
            "--cancel-resume with a blank --reason",
            vec![
                "--cancel-resume",
                APPROVAL_ID,
                "--reason",
...
/// Cancelling a resume is a tombstone write, not a resolution: its body carries
/// no disposition.
#[test]
fn cancel_resume_posts_reason_and_key_to_the_resume_cancel_route() {
    let server =
        serve(
            |request: &Request| match (request.method.as_str(), request.path.as_str()) {
                ("POST", path) if path == format!("/approvals/{APPROVAL_ID}/resume/cancel") => {
                    Response::json(200, &resume_cancellation_json())
                }
                other => panic!("unexpected request: {other:?}"),
            },
        );
    ...
    assert!(
        output.status.success(),
        "a well-formed resume cancellation must succeed; output:\n{combined}"
    );
    ...
    assert_eq!(
        recorded.len(),
        1,
        "cancel-resume is one request: {recorded:?}"
    );
    assert_eq!(
        recorded[0].header("X-Curie-Approval-Principal"),
        Some(OPERATOR_PRINCIPAL)
    );
    assert_eq!(
        body_of(&recorded[0]),
        json!({ "reason": REASON, "recovery_key": RECOVERY_KEY }),
        "a resume cancellation records no disposition; it only tombstones the resume"
    );

    let rendered: Value = serde_json::from_slice(&output.stdout)
        .unwrap_or_else(|err| panic!("--json cancel must be one object: {err}; {combined}"));
    assert_eq!(
        rendered["resume_cancelled"],
        json!({
            "approval_id": APPROVAL_ID,
            // UNCHANGED by the tombstone, which is the whole point of keeping
            // this a distinct key from `resolved`.
            "status": "approved",
            "recovery_key": RECOVERY_KEY,
            "reason": REASON,
            "actor": "U0OPERATOR",
        ...
        "the machine output must be a typed, labelled result a script cannot confuse with a resolution: {rendered}"
    );
}

/// The server's idempotency is keyed on the operator's recovery key, so the
/// CLI must send the second invocation byte-identically. A key the CLI decorated
/// with a nonce, a timestamp or an attempt counter would defeat it silently.
#[test]
fn a_retried_recover_sends_an_identical_body() {
    let server =
        serve(
            |request: &Request| match (request.method.as_str(), request.path.as_str()) {
                ("POST", path) if path == format!("/approvals/{APPROVAL_ID}/recover") => {
    ...
    for attempt in 1..=2 {
        ...
        assert!(
            output.status.success(),
            "attempt {attempt} must succeed; output:\n{}",
            text(&output)
        );
    }

    let recorded = server.recorded();
    assert_eq!(recorded.len(), 2, "two invocations, two requests");
    assert_eq!(
        recorded[0].path, recorded[1].path,
        "a replay targets the same row"
    );
    assert_eq!(
        body_of(&recorded[0]),
        body_of(&recorded[1]),
        "the replay body must be identical so the server's recovery_key idempotency can absorb it"
    );
    assert_eq!(
        body_of(&recorded[1])["recovery_key"],
        json!(RECOVERY_KEY),
        "the CLI must send the operator's key unmodified"
    );
}

/// The client is pinned to the routes' ACTUAL response models. If it ever
/// decodes something else -- an approval record, say, whose `id`, `author`,
/// `conversation_id` and `summary` these routes never send -- the mismatch must
/// surface as a loud failure here rather than only in production, where it
/// arrives AFTER the mutation has committed and tells the operator a landed
/// recovery failed.
#[test]
fn a_response_that_is_not_the_routes_model_fails_loudly() {
    let record_shaped = format!(
        r#"{{"id":"{APPROVAL_ID}","author":"U0AUTHOR","route":"explicit-reviewers","gate_kind":"policy","granted_tool":"Bash","status":"rejected","conversation_id":"thread-1","summary":"approve release","expires_at":null,"resolved_by":null,"card_channel":null}}"#
    );
    for (verb, route) in [
        ("--recover", format!("/approvals/{APPROVAL_ID}/recover")),
        (
            "--cancel-resume",
            format!("/approvals/{APPROVAL_ID}/resume/cancel"),
        ),
    ] {
        let body = record_shaped.clone();
        let expected = route.clone();
        let server = serve(move |request: &Request| {
            assert_eq!(request.path, expected, "wrong route");
        ...
        assert_eq!(
            output.status.code(),
            Some(1),
            "{verb} must not silently accept a body that is not the route's model; output:\n{combined}"
        );
        assert!(
            combined.contains("decoding"),
            "{verb}: the failure must name the decode, so the operator knows the act may have landed; output:\n{combined}"
        );
    }
}
...
/// A disabled grant is a configuration answer, not a membership answer. The
/// operator must read the setting name and the blast-radius sentence, so the
/// API's message is surfaced verbatim enough to act on.
#[test]
fn a_disabled_grant_403_is_surfaced_verbatim_and_never_as_a_membership_failure() {
    let disabled = "approval recovery is not enabled on this installation; set `api.approvalRecovery.enabled` (this grants every platform-key holder installation-wide rejection and resume cancellation, including on ordinarily resolvable approvals, and every use is audited).";
    let server = serve(move |_request: &Request| {
        Response::json(403, &json!({ "detail": disabled }).to_string())
    });
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "a refused-by-the-platform recovery is a runtime failure, not a usage error; output:\n{combined}"
    );
    assert!(
        combined.contains("api.approvalRecovery.enabled"),
        "the operator must read the SETTING NAME to act on; output:\n{combined}"
    );
    assert!(
        combined.contains("audited"),
        "the blast-radius sentence must survive to the terminal; output:\n{combined}"
    );
    let lowered = combined.to_lowercase();
    assert!(
        !lowered.contains("not a member")
            && !lowered.contains("membership")
            && !lowered.contains("not an approver"),
        "a disabled grant must never be reported as a membership problem; output:\n{combined}"
    );
}
...
/// `--dry-run` prints the request line it WOULD issue and issues nothing.
#[test]
fn dry_run_prints_the_request_line_without_issuing_it() {
    let server = no_request_server();
    let cases: &[(Vec<&str>, &str)] = &[
        (vec!["--report-identity"], "GET"),
        (
            vec![
                "--recover",
                APPROVAL_ID,
    ...
    for (args, expected) in cases {
        ...
        argv.push("--dry-run");
        let output = run("local", &argv, Some(&server), Some(OPERATOR_PRINCIPAL));
        let combined = text(&output);
        assert!(
            output.status.success(),
            "a dry run must succeed; output:\n{combined}"
        );
        assert!(
            combined.contains(expected),
            "the dry run must print the exact request it would issue (expected {expected}); output:\n{combined}"
        );
    }
    assert!(
        server.recorded().is_empty(),
        "a dry run issues nothing over the wire"
    );
}

/// The skill tier keeps no durable approval store, so all three verbs are
...
#[test]
fn the_skill_tier_declines_all_three_verbs_with_the_durable_store_reason() {
    let cases: &[Vec<&str>] = &[
        vec!["--report-identity"],
        vec![
            "--recover",
