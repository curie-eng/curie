...
fn published_v089_rollback_scenario_is_strict_and_keeps_supported_rollback() {
    ...
    assert_eq!(
        catalog["windows"]["0.10.0"]["schema_head"], "0053",
    ...
    assert!(
        ...
        "scenario must require a nonzero range refusal naming 0.8.9, published head 0039, and candidate head 0053 while rejecting identity failures"
    );
    assert!(
