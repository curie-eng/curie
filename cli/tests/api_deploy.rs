...
/// `channels` key (ADR-0118). `repo` emits the `repo_full_name` key only when
...
async fn try_deploy(
    ...
    run_deploy_result(client, channel, repo).await
}
...
async fn redeploy_with_a_new_channel_adds_a_binding() {
    // ENSURE-BOUND (ADR-0118, D3): an agent already on C0EXAMPLE1 plus
...
async fn deploy_binds_the_repo_while_also_adding_the_channel() {
    // Two writes now, not one (ADR-0118): the binding add is a POST to the
