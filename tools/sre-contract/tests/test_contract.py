...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tier", "schema"),
        ("schema", "schema"),
        ("secret-files-type", "connectors: connectors.invalid"),
        ("secrets-type", "connectors: connectors.invalid"),
        ("build-platforms", "connectors: connectors.invalid"),
        ("kubeconfig-path", "secret_files destination"),
    ],
)
def test_deliberate_mutations_reject_then_restore(healthy, mutation, expected):
    ...
    elif mutation == "secret-files-type":
        data = yaml.safe_load(paths[1].read_text())
        data["connectors"]["kubernetes"]["secret_files"] = 42
        paths[1].write_text(yaml.safe_dump(data))
    elif mutation == "secrets-type":
        data = yaml.safe_load(paths[1].read_text())
        data["connectors"]["grafana"]["secrets"] = 42
        paths[1].write_text(yaml.safe_dump(data))
    elif mutation == "build-platforms":
        data = yaml.safe_load(paths[1].read_text())
        del data["connectors"]["tempo"]["build"]["platforms"]
        paths[1].write_text(yaml.safe_dump(data))
    elif mutation == "kubeconfig-path":
        data = yaml.safe_load(paths[1].read_text())
        data["connectors"]["kubernetes"]["secret_files"] = {"K8S_KUBECONFIG": "/secrets/wrong-path"}
        paths[1].write_text(yaml.safe_dump(data))
    else:
