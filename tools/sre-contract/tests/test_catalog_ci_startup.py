...
def test_catalog_environment_follows_declared_wiring():
    grafana = {
        "env": {"GRAFANA_URL": "http://grafana.observability.svc.cluster.local"},
        "secrets": [{"name": "GRAFANA_SERVICE_ACCOUNT_TOKEN"}],
    }
    environment = catalog_ci.catalog_environment(grafana)
    assert environment["GRAFANA_URL"] == "http://127.0.0.1:9"
    assert environment["GRAFANA_SERVICE_ACCOUNT_TOKEN"] == "inert-catalog-fixture"
    unwired_spec = {"env": {"GRAFANA_URL": "http://grafana.example"}}
    unwired = catalog_ci.catalog_environment(unwired_spec)
    assert "GRAFANA_SERVICE_ACCOUNT_TOKEN" not in unwired
    assert not catalog_ci.kubeconfig_mount_needed(unwired_spec)
    kubernetes = {"secret_files": {"K8S_KUBECONFIG": "/secrets/kubeconfig"}}
    assert catalog_ci.kubeconfig_mount_needed(kubernetes)
    assert catalog_ci.kubeconfig_mount_needed(
        {"env": {"SELF_UPGRADE_KUBECONFIG": "/secrets/kubeconfig"}}
    )
    mutated = catalog_ci.volume_args(
        {"secret_files": {"K8S_KUBECONFIG": "/secrets/wrong-path"}},
        "/tmp/kube",
    )
    assert "/tmp/kube:/secrets/wrong-path:ro" in mutated
    assert "/tmp/kube:/secrets/kubeconfig:ro" not in mutated
