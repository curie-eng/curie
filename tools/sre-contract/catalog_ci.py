...
def declared_secret_names(spec):
    names = []
    for secret in spec.get("secrets") or []:
        if isinstance(secret, dict) and secret.get("name"):
            names.append(str(secret["name"]))
        elif isinstance(secret, str) and secret:
            names.append(secret)
    return names


def catalog_environment(spec):
    """Inert fixture values only for keys the connector declaration actually names."""
    environment = {}
    for key, value in dict(spec.get("env") or {}).items():
        # Catalog listing must not depend on cluster DNS. Rewrite only a
        # declared Grafana URL; never invent an undeclared credential.
        environment[str(key)] = "http://127.0.0.1:9" if str(key) == "GRAFANA_URL" else str(value)
    for name in declared_secret_names(spec):
        environment[name] = "inert-catalog-fixture"
    return environment


def volume_args(spec, kubeconfig):
    """Mount the fixture kubeconfig at every declared secret-file destination."""
    args = []
    seen = set()
    for dest in dict(spec.get("secret_files") or {}).values():
        dest = str(dest)
        if dest and dest not in seen:
            seen.add(dest)
            args += ["-v", f"{kubeconfig}:{dest}:ro"]
    for value in catalog_environment(spec).values():
        if str(value).startswith("/secrets/") and value not in seen:
            seen.add(value)
            args += ["-v", f"{kubeconfig}:{value}:ro"]
    return args


def kubeconfig_mount_needed(spec):
    return bool(volume_args(spec, "/tmp/kubeconfig"))
...
""")
            ...
            for name, spec in yaml.safe_load((BUNDLE / "connectors.yaml").read_text())[
                ...
                environment = catalog_environment(spec)
                for key, value in environment.items():
                    args += ["-e", f"{key}={value}"]
                args += volume_args(spec, kubeconfig)
