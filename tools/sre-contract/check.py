...
def validate(bundle):
    ...
    _, connector_errors = validate_connectors(connectors)
    errors += [f"connectors: {code}" for code, _message in connector_errors]
    declared = connectors["connectors"]
    kubernetes = declared.get("kubernetes") or {}
    kube_args = [str(item) for item in kubernetes.get("args") or []]
    secret_files = kubernetes.get("secret_files")
    if "--kubeconfig" in kube_args and isinstance(secret_files, dict):
        index = kube_args.index("--kubeconfig")
        if index + 1 < len(kube_args):
            path = kube_args[index + 1]
            destinations = [str(value) for value in secret_files.values()]
            if path not in destinations:
                errors.append(
                    "kubernetes kubeconfig path is not a declared secret_files destination"
                )
