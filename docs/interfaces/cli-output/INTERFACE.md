...
kind: CLEAN
impls: 52 outputs behind one trait
...
> **Kind:** CLEAN &nbsp;·&nbsp; **Implementations today:** 52 outputs behind one trait &nbsp;·&nbsp; **Swap-readiness grade:** not separately graded
...
52 `CliOutput` implementations, all in the CLI crate, grouped by owning module:
...
- **`cli/src/commands.rs`**, the largest group, covering the skill and agent verbs
  ...
  `SkillApprovalsOutput`, `OverridesOutput`, `PublicationPolicyOutput`, `WorkItemsOutput`, `ChannelsOutput`,
...
- **`cli/src/message.rs`**: `MessageDryRunOutput` and `MessageOutcomeOutput`, the
  ...
  Known leakage).
- **`cli/src/examples.rs`**: `ObservabilityProvisionOutput`, the receipt that Secret `curie-grafana-connector` holds a token. The JSON names the secret and the key. It never includes the token.
...
- **Committed JSON Schemas with a drift gate (since #841).** Each `to_json` is no
  longer schema-free: there are 53 committed schemas under `cli/schema/` with an
