...
kind: CLEAN
impls: 54 outputs behind one trait
...
> **Kind:** CLEAN &nbsp;·&nbsp; **Implementations today:** 54 outputs behind one trait &nbsp;·&nbsp; **Swap-readiness grade:** not separately graded
...
54 `CliOutput` implementations, all in the CLI crate, grouped by owning module:
...
- **`cli/src/commands.rs`**, the largest group, covering the skill and agent verbs
  ...
  `SchedulesOutput`, `HookFireOutput`, `ChannelsOutput`,
...
- **Committed JSON Schemas with a drift gate (since #841).** Each `to_json` is no
  longer schema-free: there are 55 committed schemas under `cli/schema/` with an
