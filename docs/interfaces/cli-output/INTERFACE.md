...
- **Committed JSON Schemas with a drift gate (since #841).** Each `to_json` is no
  longer schema-free: there are 44 committed schemas under `cli/schema/` with an
  index (`cli/schema/index.json`), a `syn`-based inventory gate over every `impl
  CliOutput`, and per-family output validation — all 44 are validated against real
