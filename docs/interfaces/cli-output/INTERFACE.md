...
- **Committed JSON Schemas with a drift gate (since #841).** Each `to_json` is no
  ...
  CliOutput`, and per-family output validation — all 40 result schemas are
  validated against real `to_json()` output across 66 tests in
  `cli/tests/json_contract.rs`. Those tests
