...
- `toolPolicy` (`{enforcement, allow, approvalRequired, deny}` glob collections over canonical
  ...
  **Curie's own platform-owned MCP servers, `curie` and `curie-state`, are outside `toolPolicy`
  ...
  cannot express a policy over them, and the runtime exempts them by server through
  `runner/src/curie_runner/approval.py::platform_server_for_tool` before `classify_tool` is ever
  consulted. A policy therefore cannot remove the approval, publication or channel-memory paths,
  and a literal pattern naming one of those servers that the bundle does not declare is a
  deploy-time error (`tool_policy.platform_server`). The cost, stated rather than hidden: a bundle
