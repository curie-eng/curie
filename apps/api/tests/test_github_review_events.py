...
@pytest.mark.parametrize(
    "claimed_url",
    [
        f"https://github.com/{REPO}/pull/17#issuecomment-71",
        f"https://github.com/{REPO}/issues/17#issuecomment-71",
    ],
)
def test_pull_request_issue_comment_url_shapes_normalize_to_one_identity(
    claimed_url: str,
) -> None:
    # The /pull form was observed through GitHub REST for this repo's PRs #2616
    # and #2560. The provider's general issue-comment example uses /issues:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        f"https://github.com/{REPO}/issues/17#issuecomment-72",
    ],
)
def test_issue_comment_url_aliases_require_exact_repository_pr_and_comment(
    claimed_url: str,
) -> None:
    payload = feedback_payload()
    payload["comment"]["html_url"] = claimed_url
...
@pytest.mark.parametrize("permission_case", [
    "member", "allowed", "revoked", "wrong_id", "bad_login", "http404", "http403", "timeout",
...
def test_final_reserve_rereads_provider_head_before_creating_reservation(
    review_stack,
) -> None:
    client, truth, valkey, stream = review_stack
    base, payload, headers = _review_reserve_request(review_stack)
    lineage_before = review_rows(
        "SELECT head_sha,version FROM curie.thread_publication_lineages"
    )

    # Only provider truth moves after preliminary verification. The persisted
    # lineage and caller's verified version/head remain byte-for-byte current,
    # so a DB-only reserve check would incorrectly authorize the model turn.
    truth.pr["head"]["sha"] = "b" * 40
    truth.calls.clear()
    result = client.post(base + "/reserve", json=payload, headers=headers)

    assert result.status_code == 409, result.text
    assert result.json()["detail"]["code"] == "stale_feedback_head"
    assert f"/repos/{REPO}/pulls/17" in truth.calls
    assert review_rows(
        "SELECT head_sha,version FROM curie.thread_publication_lineages"
    ) == lineage_before
...
def test_final_reserve_provider_timeout_is_retryable_without_reservation(
    review_stack,
) -> None:
    client, truth, valkey, stream = review_stack
    base, payload, headers = _review_reserve_request(review_stack)
    ...
    def provider_timeout(request: httpx.Request) -> httpx.Response:
        if request.url.path == pull_path:
            truth.calls.append(request.url.path)
            raise httpx.ReadTimeout("synthetic final-reserve timeout", request=request)
        return truth.handle(request)

    original_http = client.app.state.http_client
    injected = httpx.AsyncClient(transport=httpx.MockTransport(provider_timeout))
    client.app.state.http_client = injected
    truth.calls.clear()
    try:
        result = client.post(base + "/reserve", json=payload, headers=headers)
    ...
    assert pull_path in truth.calls
    assert review_rows(
        "SELECT head_sha,version FROM curie.thread_publication_lineages"
    ) == lineage_before
    assert review_rows("SELECT id FROM curie.publication_review_reservations") == []
