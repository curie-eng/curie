...
def test_patch_onto_taken_address_is_409(
    ...
    (ADR-0118), so the move goes there; the 409 it must answer is unchanged."""
...
def test_a_non_slack_kind_binds_and_reads_back_through_the_api(
    ...
    The read-back asserts the binding surface is a LIST. ADR-0118 amends
...
def test_patching_a_binding_moves_it_rather_than_adding_a_second(
    ...
    """T4, rewritten for ADR-0118: the two verbs stay distinct.
...
def test_a_singular_channel_patch_is_rejected_not_silently_ignored(
    ...
    ADR-0118, and PATCH is where saying so matters. Pydantic's default
...
def test_the_published_agent_update_has_no_channel_property_at_all() -> None:
    ...
    `null` as a value the server refused. ADR-0118 removes the field outright,
    ...
    assert "channel" not in update, (
        ...
        f"(ADR-0118). Published properties: {sorted(update)}"
    )
