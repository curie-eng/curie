...
`curie_api.migration_fence` is a helper imported BY revisions 0022, 0024 and
0045 (importable because `alembic.ini` sets `prepend_sys_path = src`). It does
...
def test_a_concurrent_binding_update_blocks_on_the_fence_and_then_succeeds(
    isolated_migration_db: None,
) -> None:
    """The fence's negative control, and the binding-mutation half of AC2.

    0022 never ALTERs `curie.agent_channels`, so nothing but
    `fence_identity_tables` can make this UPDATE wait. Delete the fence and
    `_await_waiting_on_lock` fails: the re-point lands mid-migration, between
    the preflight's answer and the backfill that records it, and 0022 persists
    the LATER kind as the approval's original reply identity. That is the
    silent misroute the column exists to prevent.
    """

    cfg = _at(BELOW_0022)
    _seed_binding("slack-agent", "slack", "C0EXAMPLE1")
    approval = _seed_approval(reply_channel="C0EXAMPLE1")
    _install_version_stamp_pause(REVISION_0022)

    repoint = "UPDATE curie.agent_channels SET kind = 'email' WHERE address = 'C0EXAMPLE1'"

    with ThreadPoolExecutor(max_workers=2) as pool:
        migration = pool.submit(command.upgrade, cfg, REVISION_0022)
        _await_paused_migration(migration)
        probe = pool.submit(_run_statement, repoint)
        _await_waiting_on_lock(probe, "UPDATE curie.agent_channels")
        migration.result(timeout=BOUND_SECONDS)
        probe.result(timeout=BOUND_SECONDS)

    # The re-point was admitted, not refused...
    assert _sql("SELECT kind FROM curie.agent_channels WHERE address = 'C0EXAMPLE1'") == [
        ("email",)
    ]
    # ...and the approval kept the kind it was RAISED on, because the binding
    # could not move inside the fence.
    assert _reply_identity(approval) == ("slack", None)


def test_0045_takes_the_fence_on_an_install_already_past_0024(
    isolated_migration_db: None,
) -> None:
    """0045 must take the identity fence itself (#2753).

    An install already past 0024 runs ONLY 0045 on this upgrade, so 0022's and
    0024's fences never fire there. 0045 ALTERs `curie.approvals` only, so the
    one thing that can make an `agent_channels` UPDATE wait is 0045 calling
    `fence_identity_tables`. Remove that call and `_await_waiting_on_lock`
    fails because the UPDATE lands mid-migration.
    """

    cfg = _at(BELOW_0045)
    _seed_binding("slack-agent", "slack", "C0EXAMPLE1")
    _install_version_stamp_pause(REVISION_0045)

    repoint = "UPDATE curie.agent_channels SET kind = 'email' WHERE address = 'C0EXAMPLE1'"

    with ThreadPoolExecutor(max_workers=2) as pool:
        migration = pool.submit(command.upgrade, cfg, REVISION_0045)
        _await_paused_migration(migration)
        probe = pool.submit(_run_statement, repoint)
        _await_waiting_on_lock(probe, "UPDATE curie.agent_channels")
        migration.result(timeout=BOUND_SECONDS)
    ...
    assert _stamped_revision() == REVISION_0045
    assert _sql("SELECT kind FROM curie.agent_channels WHERE address = 'C0EXAMPLE1'") == [
        ("email",)
    ]


def test_a_concurrent_select_on_approvals_blocks_for_the_fence_and_then_succeeds(
    isolated_migration_db: None,
) -> None:
    """ACCESS EXCLUSIVE is a reader fence too, and this test says so.

    It used to assert the opposite -- that a reader sails past, because SHARE
    ROW EXCLUSIVE does not conflict with ACCESS SHARE. That claim is no longer
    true, and it is not true by CHOICE: a fence weak enough to let a reader
    through is a fence the revision's own ACCESS EXCLUSIVE DDL has to upgrade
    from, and that upgrade deadlocks against a reader that then writes (see
    `test_a_reader_that_becomes_a_writer_is_not_deadlocked_by_the_fence`). A
    dashboard or `GET /approvals` stalling for the migration's duration is a
    brief, bounded wait; aborting a live resolution is not.

    So the property asserted is the same one every other concurrent probe here
    asserts: it BLOCKS and then SUCCEEDS. It is never refused.

    Driven against 0024, which ALTERs `agent_channels` only, so the block on
    `curie.approvals` is caused by `fence_identity_tables` and by nothing else.
    """

    cfg = _at(BELOW_0024)
    _seed_binding("slack-agent", "slack", "C0EXAMPLE1")
    _seed_approval(reply_channel="C0EXAMPLE1", reply_kind="slack")
    _install_version_stamp_pause(REVISION_0024)

    with ThreadPoolExecutor(max_workers=2) as pool:
        migration = pool.submit(command.upgrade, cfg, REVISION_0024)
        _await_paused_migration(migration)
        reader = pool.submit(_run_statement, "SELECT count(*) FROM curie.approvals")
        _await_waiting_on_lock(reader, "SELECT count(*) FROM curie.approvals")
        # The fence really was held over that read in the strongest mode, so
        # this is not a reader parked on somebody else's lock.
        held = _sql(
            """
            SELECT 1 FROM pg_locks l
            JOIN pg_class c ON c.oid = l.relation
            JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE n.nspname = 'curie' AND c.relname = 'approvals'
              AND l.mode = 'AccessExclusiveLock' AND l.granted
            """
        )
        assert held, "the fence never took ACCESS EXCLUSIVE on curie.approvals"
        migration.result(timeout=BOUND_SECONDS)
        # Waited, then won. Not refused.
        reader.result(timeout=BOUND_SECONDS)
...
def _channels_then_approvals(channels_done: Any, proceed: Any, *, address: str) -> None:
    """`crud.delete_agent`'s lock order, in one transaction.

    It deletes the agent's `agent_channels` rows (ROW EXCLUSIVE, held to
    commit) and only then deletes the agent, whose cascade reaches
    `curie.approvals`. Publication create takes the same two tables in the same
    order. The pause between the two statements is where the fence arrives.
    """
    ...
    async def _go() -> None:
        ...
        try:
            async with engine.begin() as conn:
                ...
                await conn.execute(
                    text("DELETE FROM curie.agent_channels WHERE address = :addr"),
                    {"addr": address},
                )
                channels_done.set()
                while not proceed.is_set():
                    await asyncio.sleep(0.02)
                await conn.execute(
                    text("DELETE FROM curie.approvals WHERE reply_channel = :addr"),
                    {"addr": address},
                )
        finally:
            await engine.dispose()
...
def test_a_channels_then_approvals_writer_is_not_deadlocked_by_the_fence(
    isolated_migration_db: None,
) -> None:
    """The writer-side cycle, which ONLY the fence's lock order prevents.

    `LOCK TABLE a, b` takes `a` and then `b`; it is not simultaneous. Fencing
    `approvals` first lets the fence be granted `approvals` while this writer
    holds `agent_channels`; the fence then waits on `agent_channels` and the
    writer's cascade into `approvals` waits on the fence -- a cycle PostgreSQL
    breaks by aborting one side. Fencing `agent_channels` first makes the fence
    wait on the writer before it holds anything, so the writer finishes and the
    fence follows.

    Swap `FENCED_TABLES` back to `(approvals, agent_channels)` and this fails
    with `DeadlockDetected` from one of the two `result()` calls.
    """
    ...
    cfg = _at(BELOW_0022)
    _seed_binding("slack-agent", "slack", "C0EXAMPLE1")
    kept = _seed_approval(reply_channel="C0EXAMPLE1")
    # A second agent the writer deletes, so the migration's own row is untouched.
    _seed_binding("doomed-agent", "slack", "C0DOOMED")
    doomed = _seed_approval(reply_channel="C0DOOMED")

    channels_done = threading.Event()
    proceed = threading.Event()

    with ThreadPoolExecutor(max_workers=2) as pool:
        writer = pool.submit(
            _channels_then_approvals, channels_done, proceed, address="C0DOOMED"
        )
        assert channels_done.wait(timeout=BOUND_SECONDS), "the writer never started"

        migration = pool.submit(command.upgrade, cfg, REVISION_0022)
        deadline = time.monotonic() + BOUND_SECONDS
        while not _sql(
            "SELECT 1 FROM pg_stat_activity WHERE wait_event_type = 'Lock' "
            "AND datname = current_database() AND query LIKE 'LOCK TABLE%'"
        ):
            if migration.done():
                migration.result()
                pytest.fail("the migration finished before the writer could contend")
            assert time.monotonic() < deadline, "the fence never contended"
            time.sleep(0.02)

        proceed.set()
        writer.result(timeout=BOUND_SECONDS)
        migration.result(timeout=BOUND_SECONDS)

    assert _stamped_revision() == REVISION_0022
    assert _sql("SELECT id FROM curie.approvals WHERE id = :id", {"id": doomed}) == []
    assert _reply_identity(kept) == ("slack", None)
...
def test_a_declaration_naming_an_unknown_approval_is_refused(
    isolated_migration_db: None, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A stale declaration left mounted after an upgrade cannot take effect.

    It produces a loud refusal, not a silent re-application -- the property
    that makes a single-use Secret safe to forget about.
    """
    ...
    _write_declarations(
        ...
        monkeypatch,
        [
            _declaration(ambiguous, reply_kind="sms", reply_adapter="twilio-main"),
            _declaration(stale, reply_kind="sms", reply_adapter="twilio-main"),
        ],
    )
    ...
    assert str(stale) in str(caught.value), str(caught.value)
    assert _stamped_revision() == BELOW_0022
...
@pytest.mark.parametrize(
    "broken,label",
    [
        pytest.param("{ not json at all", "unparseable", id="unparseable"),
        pytest.param(None, "actor", id="missing-actor"),
        pytest.param(None, "reason", id="missing-reason"),
        pytest.param(None, "reply_kind", id="missing-reply-kind"),
    ],
)
def test_a_malformed_declaration_is_refused_and_never_partially_applied(
    isolated_migration_db: None,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    broken: str | None,
    ...
    """Every field is load-bearing, so a half-filled skeleton is not a weaker
    declaration -- it is not one.

    The operator's artifact is a template they fill in by hand, and the failure
    this forbids is the one where they fill in two of four fields, the
    migration honors it anyway, and the audit row that was supposed to
    attribute the bypass names nobody. The refusal must therefore name the
    DOCUMENT's defect -- the offending row and the field, or the file that
    would not parse -- not merely re-report the row as unreconstructable, which
    is the answer the operator already had and tells them nothing about what to
    fix.

    A second, WELL-FORMED declaration sits alongside the broken one so the
    all-or-nothing clause has something to be true about: it must not be
    honored, and the good row must come out of this with no reply identity and
    no audit row.
    """

    cfg = _at(BELOW_0022)
    good = _seed_unreconstructable()
    other = _seed_approval(reply_channel="nobody@example.test", summary="also orphaned")
    good_declaration = _declaration(good, reply_kind="sms", reply_adapter="twilio-main")
    if broken is not None:
        _write_declarations(tmp_path, monkeypatch, broken)
        path = tmp_path / "provenance-declarations.json"
    else:
        payload = _declaration(other, reply_kind="sms", reply_adapter="twilio-main")
        payload.pop(label)
        path = _write_declarations(tmp_path, monkeypatch, [good_declaration, payload])

    with pytest.raises(RuntimeError) as caught:
        command.upgrade(cfg, REVISION_0022)

    message = str(caught.value)
    # The document is always named, so the operator knows WHICH file to fix --
    # the refusal is about the declaration, not about the row.
    assert str(path) in message, message
    if broken is None:
        assert str(other) in message, message
        assert label in message, message

    # All or nothing: the well-formed declaration in the same file was NOT
    # applied, and nothing was stamped or written.
    assert _stamped_revision() == BELOW_0022
    assert "reply_kind" not in _approvals_columns()
    assert _audit_rows(good) == []
    assert _audit_rows(other) == []
...
@pytest.mark.parametrize(
    "reply_kind,reply_adapter,needle",
    [
        pytest.param("email", None, "null 'reply_adapter'", id="non-slack-without-adapter"),
        pytest.param("slack", "smtp-primary", "must be null", id="slack-with-adapter"),
    ],
)
def test_a_declaration_whose_kind_and_adapter_disagree_is_refused(
    isolated_migration_db: None,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    reply_kind: str,
    ...
    """A declaration must actually satisfy the obligation it is clearing.

    `{"reply_kind": "email", "reply_adapter": null}` used to validate. 0024 then
    dropped the row from its refusal set because a declaration was honored,
    although the missing egress identity that raised the guard was still
    missing: the migration went green and the eventual reply failed inside the
    worker for want of a credential, days later and far from anything an
    operator could connect it to. That is the guard being laundered, not
    satisfied. The mirror case is a Slack row with an adapter -- a credential
    selector the Slack egress never consults.

    Refused at document-validation time, so nothing in the file is applied.
    """
    ...
    _write_declarations(
        tmp_path,
        monkeypatch,
        [_declaration(ambiguous, reply_kind=reply_kind, reply_adapter=reply_adapter)],
    )
...
def test_0024_recovers_only_the_adapter_and_records_what_it_already_knew(
    isolated_migration_db: None, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The agreeing case, asserted on the AUDIT ROW as well as the approval.

    The kind is not re-written even though the declaration names it: the audit
    entry records the kind 0022 established, so the record says which half of
    the identity was actually recovered here.
    """
    ...
    _write_declarations(
        tmp_path,
        monkeypatch,
        [_declaration(email_approval, reply_kind="email", reply_adapter="smtp-primary")],
    )
    command.upgrade(cfg, REVISION_0024)
    ...
    honored = [r for r in _audit_rows(email_approval) if r.action == HONORED_ACTION]
    assert len(honored) == 1
    assert honored[0].evidence["established_reply_kind"] == "email"
...
def test_a_non_slack_row_recovered_at_0022_carries_its_adapter_past_0024(
    isolated_migration_db: None, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The other half of the single-document workflow.

    A row 0022 recovered to a NON-Slack kind already carries its egress identity
    on the approval, which is where `resumequeue` reads it from. 0024's refusal
    set must therefore not contain it -- it is routable -- and the still-mounted
    declaration must not be honored a second time.
    """

    # An orphan rather than `_seed_unreconstructable`'s two-kinds-at-one-address
    # row: that helper drops `agent_channels_address_key`, which a later
    # revision in this graph expects to still be there, and this test walks the
    # WHOLE graph.
    cfg = _at(BELOW_0022)
    ambiguous = _seed_approval(reply_channel="ops@example.test", summary="no binding")
    _write_declarations(
        tmp_path,
        monkeypatch,
        [_declaration(ambiguous, reply_kind="sms", reply_adapter="twilio-main")],
    ...
    assert _reply_identity(ambiguous) == ("sms", "twilio-main")
    honored = [r for r in _audit_rows(ambiguous) if r.action == HONORED_ACTION]
    assert len(honored) == 1, honored
    assert honored[0].evidence["revision"] == REVISION_0022
