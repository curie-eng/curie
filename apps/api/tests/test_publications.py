...
def test_review_reservation_refuses_legacy_unproved_lineage(
    ...
    _, publication = _open_lineage(
        client,
        auth_headers,
        conversation_id="legacy-null-authority",
    )
...
def test_post_capture_publication_binds_route_then_first_advance_binds_github_identity(
    review_lineage_app: tuple[TestClient, dict[str, Any], str],
    auth_headers: dict[str, str],
) -> None:
    client, truth, _ = review_lineage_app
    deployment = _create_deployment(client, auth_headers)
    bare_conversation = "1700000000.000100"
    scoped_conversation = channel_protocol.scoped_conversation_id(
        "slack", "C0EXAMPLE1", bare_conversation
    )
    payload = _publication_payload(
        deployment["id"],
        conversation_id=scoped_conversation,
        dedupe_key="review-authority-capture",
    )
    payload["reply_conversation_id"] = bare_conversation
    ...
    assert captured == [
        {
            "binding_id": captured[0]["current_binding_id"],
            "binding_generation": captured[0]["current_generation"],
            "reply_conversation_id": bare_conversation,
            "conversation_id": scoped_conversation,
