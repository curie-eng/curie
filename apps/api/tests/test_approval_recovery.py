"""Break-glass approval recovery (#2753): enablement, atomicity, report.
...
Two routes are under test, on a router separate from ``routers/approvals.py``:
...
* ``POST /approvals/{id}/recover``        -- administrative disposition, one
  transaction, one commit, replay-safe on the ``recovery_key`` its audit row
  records.
...
def _approval_row(approval_id: str) -> dict[str, Any]:
    rows = _sql(
        "SELECT status, reply_kind, reply_channel, reply_placeholder, "
        "resolved_at, resumed_at FROM curie.approvals WHERE id = :id",
...
def test_disabled_grant_refuses_recovery_naming_the_setting(
    ...
    # Nothing happened: still pending, no audit row.
...
def test_a_key_already_recorded_for_another_approval_is_a_conflict(
    recovery_client: TestClient,
    auth_headers: dict[str, str],
    clean_db: None,
    valkey: redis.Redis,
    ...
    """A key names one administrative act. Reusing it on a second approval is
    refused before anything changes, so a replay can never land on the wrong row."""

    first = _channel_membership_approval(recovery_client, auth_headers)
    made = recovery_client.post(
        "/approvals",
        json=_payload(agent_id=first["agent_id"], route="managers", card_channel=_BROAD),
    ...
    ok = recovery_client.post(f"/approvals/{first['id']}/recover", json=body, headers=headers)
    assert ok.status_code == 200, ok.text

    reused = recovery_client.post(f"/approvals/{second['id']}/recover", json=body, headers=headers)
    assert reused.status_code == 409, reused.text
    assert "another approval" in reused.json()["detail"]

    assert _approval_row(second["id"])["status"] == "pending"
    assert _audit(recovery_client, auth_headers, second["id"]) == []
    assert len(valkey.xrange(runs_stream)) == 1
...
# a publication behind it, and the ordinary reject settles both.
...
def _publication_row(publication_id: str) -> dict[str, Any]:
    rows = _sql(
        "SELECT status, version, terminal_at, patch_bytes FROM curie.publications WHERE id = :id",
