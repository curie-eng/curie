...
@pytest.mark.parametrize("permission", ["write", "admin"])
def test_member_associations_require_fresh_effective_app_permission(
    review_app_key, monkeypatch, association, permission,
):
...
@pytest.mark.parametrize(
    "association", ["OWNER", "MEMBER", "COLLABORATOR", "CONTRIBUTOR"]
)
def test_no_accepted_association_can_bypass_an_unavailable_permission_check(
...
def test_association_cannot_gain_authority_from_role_name_or_unproved_permission(
    review_app_key, monkeypatch, association, permission,
):
    truth = PermissionTruth("issue_comment", review_app_key, association)
...
def test_permission_response_must_name_the_exact_fetched_immutable_user(
    review_app_key, monkeypatch, association, user,
):
    truth = PermissionTruth("issue_comment", review_app_key, association)
