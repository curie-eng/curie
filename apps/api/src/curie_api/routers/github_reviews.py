...
async def reserve_review(
    ...
    if event_id != payload.turn.event_id:
        raise HTTPException(
            409,
            {"code": "feedback_turn_mismatch"},
            headers={"Cache-Control": "no-store"},
        )
    try:
        # This outer deadline includes every provider read, DB lock/CAS, flush,
        # and commit. Per-request httpx timeouts can multiply, and ASGI test
        # transports do not enforce them. Cancelling here stops this coroutine
        # before an eventual provider response can create a late reservation.
        async with asyncio.timeout(_FINAL_RESERVE_CONTROL_PLANE_TIMEOUT_S):
        ...
        await session.rollback()
        unavailable = FeedbackUnavailable("feedback_reserve_timeout")
        raise HTTPException(
            503,
