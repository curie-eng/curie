"""Break-glass approval recovery (#2753): report, recover, cancel a resume.
...
default. Enabling it lets ANY platform-key holder reject an approval and cancel
an owed resume installation-wide, including on ordinarily resolvable approvals.
...
from ..models import AgentChannel, Approval, ApprovalStatus
...
from ..schemas import (
    ...
    ApprovalResumeCancel,
    ApprovalResumeCancelOut,
)
...
RECOVERY_DISABLED_DETAIL = (
    ...
    "it. Enabling it lets every platform-key holder reject an approval and "
    "cancel a resume installation-wide, including on ordinarily resolvable "
    "approvals, and every use is audited."
)
...
async def _binding_facts(session: SessionDep, agent_id: uuid.UUID | None) -> tuple[bool, set[str]]:
    ...
    return bool(rows), {kind for kind, adapter in rows if adapter}
...
async def recover_approval(
    ...
    The compare-and-set, the ``recovery_key`` write and the audit append are one
    transaction with one commit, so there is no window in which the status
    flipped and the row explaining it does not exist. The resume then rides the
    ...
    try:
        ...
        # The partial-unique index: this key already names another approval's
        # outcome, so it cannot also name this one.
        await session.rollback()
        raise HTTPException(
            status.HTTP_409_CONFLICT,
    ...
    if recovered is None:
        ...
        return _replayed_recovery(current, data.recovery_key)
    ...
    return _recovery_out(recovered)


def _recovery_out(approval: Approval) -> ApprovalRecoveryOut:
    return ApprovalRecoveryOut(
        approval_id=approval.id,
        status=approval.status,
        recovery_key=approval.recovery_key,
...
def _replayed_recovery(current: Approval | None, recovery_key: str) -> ApprovalRecoveryOut:
    """A lost CAS is either a replay or a conflict, and the key says which.
    ...
    if current is None:
        raise _not_found()
    if current.recovery_key == recovery_key:
        return _recovery_out(current)
    raise HTTPException(
        status.HTTP_409_CONFLICT,
        f"approval is already {current.status} and carries another administrative outcome",
    )


@router.post(
    "/{approval_id}/resume/cancel",
    response_model=ApprovalResumeCancelOut,
    dependencies=[PlatformKey, RecoveryEnabled],
)
@_deadlock_as_retryable_conflict
async def cancel_approval_resume(
    approval_id: uuid.UUID,
    data: ApprovalResumeCancel,
    session: SessionDep,
    principal: ApprovalPrincipalDep,
) -> ApprovalResumeCancelOut:
    """Tombstone an owed resume so no wake for it ever executes.

    Legal only while the record is settled and the wake is still owed. A CAS on
    ``resumed_at`` alone could not retract a stream entry that already exists,
    so the tombstone is a DATABASE veto the execution side reads, not a queue
    operation. Only a resume that has NEVER started executing can be
    cancelled: once a delivery has recorded execution, this returns 409 naming
    that record, because nothing here can prove a started execution stopped.
    The resume must be allowed to complete (the worker's reclaim of an
    orphaned delivery interrupts an orphan runner and proves it idle).
    Cancelling a started resume is a named follow-up.

    It retains everything: every reply-identity column and every
    prior audit row survives, because a tombstone nobody can explain afterwards
    is worse than the stranded resume it replaced.
    """
    ...
    try:
        with operation_span(
            "curie.approval.resume.cancel",
            kind=SpanKind.INTERNAL,
            parent=approval_trace_context(approval),
            attributes={"service.name": "curie-api", "operation": "resume_cancel"},
        ):
            cancelled = await crud.cancel_approval_resume_atomic(
                session,
                approval_id,
                reason=data.reason,
                recovery_key=data.recovery_key,
    ...
    if cancelled is None:
        ...
        if current is None:
            raise _not_found()
        if current.resume_cancelled_at is not None and current.recovery_key == data.recovery_key:
            return _cancel_out(current)
        raise HTTPException(status.HTTP_409_CONFLICT, _why_cancel_is_illegal(current))

    logger.warning(
        "resume for approval %s administratively cancelled by %s (key %s): %s",
        approval_id,
        principal.subject,
        data.recovery_key,
        data.reason,
    )
    return _cancel_out(cancelled)


def _why_cancel_is_illegal(current: Approval) -> str:
    """Name the state that refused, so the operator is not left guessing."""

    if current.resolved_at is None:
        return "approval is still pending, so no resume is owed to cancel"
    if current.resumed_at is not None:
        return "resume was already dispatched and can no longer be cancelled"
    if current.resume_cancelled_at is None and current.resume_executing_at is not None:
        return (
            "a delivery has started executing this resume (recorded "
            f"{current.resume_executing_at.isoformat()}, lease "
            f"{current.resume_executing_lease_key or 'none'}, owner "
            f"{current.resume_executing_owner or 'unrecorded'}, generation "
            f"{current.resume_executing_generation}), and cancellation cannot "
            "prove it has stopped; the resume must be allowed to complete"
        )
    if current.resume_cancelled_at is not None:
        return "resume is already cancelled under another administrative outcome"
    return "approval carries another administrative outcome"


def _cancel_out(approval: Approval) -> ApprovalResumeCancelOut:
    return ApprovalResumeCancelOut(
        approval_id=approval.id,
        status=approval.status,
        recovery_key=approval.recovery_key,
        reason=approval.resume_cancelled_reason,
        actor=approval.resume_cancelled_by,
        cancelled_at=approval.resume_cancelled_at,
    )
